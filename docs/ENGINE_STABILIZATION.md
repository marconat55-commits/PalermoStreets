...
- Added a conservative runtime-only width correction to the final `getup` frame so its transition to `idle` does not visibly change body scale. Approved PNG files remain untouched.
