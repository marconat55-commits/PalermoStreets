...
Gli attori ambientali animati usano `kind: sprite_loop`. Ogni definizione dichiara frame, posizione, dimensione, pivot, durata di ciascun frame e parallasse. Il runtime precarica i PNG insieme allo stage, li anima su un piano dietro al combattimento e li rilascia al cambio di modulo. Nel pilot M01 sono attivi `m01_venditore_frutta` e `m01_signora_balcone`. La signora usa l'alloggio `BALCONY_RESIDENT_01`: vano MAIN e ringhiera FOREGROUND condividono parallasse `1.0`, mentre l'attore viene composto fra i due. Gli stormi e il relativo renderer sono stati eliminati. Il contratto e documentato in `content-src/stages/stage1_zen/modules/M01_AMBIENT_SOCKETS.md`; fonti e SHA-256 sono registrati nei manifest v3 dell'attore e v1 dell'alloggio.
