...
Gli attori ambientali animati usano `kind: sprite_loop`. Ogni definizione dichiara frame, posizione, dimensione, pivot, durata di ciascun frame e parallasse. Il runtime precarica i PNG insieme allo stage, li anima su un piano dietro al combattimento e li rilascia al cambio di modulo. Nel pilot M01 e attivo soltanto `m01_venditore_frutta`. `m01_signora_balcone` resta conservata come sorgente, ma tornera nel runtime dopo la costruzione dell'alloggio descritto in `content-src/stages/stage1_zen/modules/M01_AMBIENT_SOCKETS.md`. Lo stage deve fornire vano MAIN e ringhiera FOREGROUND prima del personaggio. Gli stormi e il relativo renderer sono stati eliminati. Fonti, output e SHA-256 correnti sono registrati in `art_source/ambient/stage1_zen/M01/v3/M01_AMBIENT_V3_MANIFEST.json`.
