...
Gli attori ambientali animati usano `kind: sprite_loop`. Ogni definizione dichiara frame, posizione, dimensione, pivot, durata di ciascun frame e parallasse. Il runtime precarica i PNG insieme allo stage, li anima su un piano dietro al combattimento e li rilascia al cambio di modulo. Il pilot M01 contiene `m01_signora_balcone` e `m01_venditore_frutta`; fonti, output e SHA-256 correnti sono registrati in `art_source/ambient/stage1_zen/M01/v3/M01_AMBIENT_V3_MANIFEST.json`. La signora e composta come figura isolata dentro il balcone gia presente nel fondale, cosi scala e allineamento architettonico dipendono dallo stage approvato. Gli stormi procedurali non sono configurati. La direzione richiede degrado credibile da borgata popolare ed espressioni guardinghe.
